...
public final class PlayerManager implements IPlayer.PlayCallback {
    ...
    public void start(AssetFileDescriptor assetFileDescriptor, int observerHash,String url) {

        bindPlayerView(assetFileDescriptor, observerHash, url);

        onPlayStateChanged(PlayState.STATE_LOADING);
        Utils.log(String.format("start loading video, hash=%d, url=%s", mObserverHash, mVideoUrl));
        mPlayer.start(assetFileDescriptor);
    }

    void bindPlayerView(AssetFileDescriptor assetFileDescriptor, int observerHash, String url) {
