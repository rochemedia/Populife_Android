...
public class VideoPlayerView extends StandardVideoView implements
    ...
    protected void play() {
        ...
        //准备开始播放
        if (mVideoUrl.toLowerCase().startsWith("http://") || mVideoUrl.toLowerCase().startsWith("https://")){
            PlayerManager.getInstance().start(mVideoUrl, mViewHash);
        }else {
            AssetFileDescriptor fd = null;
            try {
                fd = getContext().getAssets().openFd(mVideoUrl);
                PlayerManager.getInstance().start(fd, mViewHash, mVideoUrl);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
